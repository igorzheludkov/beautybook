...
export default function OrderItem({ item, clientEmail }) {
  ...
  useEffect(() => {
    setOptions([item.serviceItem.item_1, item.serviceItem.item_2, item.serviceItem.item_3])
  }, [item])
  ...
  function clientContactsHandler(e) {
    setContacts({ ...contacts, [e.target.id]: e.target.value, visitDur: +choosenOpt.dur })
  }
      ...
      {statusMessage.status === 0 && (
        <>
          <div className='container'>
            ...
            {/* <div className={s.title_h3}>Виберіть послугу натиснувши на неї</div> */}
          </div>
            <div className={s.message}>Виберіть послугу натиснувши на неї</div>
          <div className={s.options}>
            <form className={s.options_wrapper}>
              {options?.map((i, index) => (
                <>
                  {i.name && (
                    <label key={index} className={s.options_container}>
                      <input
                        className={s.option}
                        onChange={() => setChoosenOpt(i)}
                        name='position'
                        value={i}
                        type='radio'
                      />
                      <div className={s.option_name}>{i.name}</div>
                      <div className={s.option_price}>{i.price} грн</div>
                      <div className={s.option_dur}>{i.dur} хв </div>
                      <span className={s.checkmark_options}></span>
                    </label>
                  )}
                </>
              ))}
          ...
          <div className={s.calendar_wrapper}>
            {bookedOrders && user && (
              <Calendar props={{ visitHandler, choosenOpt, user, bookedOrders, choosenTimeStamp }} />
            )}
